...
def balanced_sample_by_score(
    ...
    print(f"  Groups before sampling:")
    ...
    print(f"  → Total after balancing: {min_size * len(score_groups)} (from {len(data)})")
...
def print_final_statistics(
    ...
    if normalized_scores:
        ...
        for score in range(10):
            ...
            bar = "█" * int(count * 30 / max(score_counts.values())) if max(score_counts.values()) > 0 else ""
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float, 
        default=0.9, 
        help="Median score for normalization"
    )
    parser.add_argument(
        "--score_mad",
        type=float,
...
def apply_sampling_strategy(
    ...
    if args.balanced_sampling:
        ...
        data = balanced_sample_by_score(data, args.score_median, args.score_mad, args.seed)
...
def main():
    ...
    if args.balanced_sampling:
        print_score_distribution(original_scores, "Original Score Distribution (before sampling)")
    ...
    if args.balanced_sampling:
        print_score_distribution(sampled_scores, "Raw Score Distribution (after balanced sampling)")
